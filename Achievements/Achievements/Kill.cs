...
class KillFactory : AchievementFactory {
...
class Kill : CountingAchievement {
	private string title;
	private string text;
	private bool resetOnVesselChange;
	private HashSet<string> killedCrewNames = new HashSet<string>();

	public Kill(int minKilled, string title, string text, bool resetOnVesselChange) : base(minKilled) {
		this.title = title;
		this.text = text;
...
class KillJebediahAgain : CountingAchievement {
	private bool killed;

	public KillJebediahAgain() : base(2) {
